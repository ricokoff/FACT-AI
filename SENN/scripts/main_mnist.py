...
from SENN.utils import plot_theta_stability, generate_dir_names, noise_stability_plots, concept_grid, plot_digit, plot_concept_vis
...
def parse_args():
    ...
    parser.add_argument('--autovis', action='store_true', default=False, help='Visualize the autoencoder / activations' )
...
def main():
    ...
    if args.autovis:
        ...
        with torch.no_grad():
            plot_concept_vis(model, args.nconcepts, 5, 4)
        exit()

    # print(next(iter(test_tds))[0].shape)
    # exit()
    ...
    #     model.eval()
    #
    #     accum = get_concept_vis(model, args.nconcepts)
    #     print(accum)
    #     print(model)
    #     x = next(iter(test_tds))[0]
    #     x = test_tds[1][0]
    #     print(x.shape)
    #     plot_digit()
    #     plot_digit(x)
    ...
    #     print(x.size())
    #     with torch.no_grad():
    #          enc = model.conceptizer.encode(x).squeeze()[:, None]
    #          print(enc.shape)
    #          plot_digit(enc)
    #     plt.show()
    #
    ...
    pickle.dump(All_Results, open(results_path + '_combined_metrics.pkl', "wb"))
